...
```{r}
mod.clogit.ns.3 <- clogit(Case ~ ns(dailyPM, df = 3) + ns(dailyTemp, df=3) + 
                          ns(dailyRH, df=4) + strata(SubjectID), 
                          method = "efron", data = dta)

ptemp <- predict(mod.clogit.ns.3, type = "terms", se = TRUE) %>% as.data.frame()
colnames(ptemp) <- c("termPM.ns.3", "termTemp.ns.3", "termRH.ns.3", 
                     "sePM.ns.3", "seTemp.ns.3", "seRH.ns.3")
dta.pred <- bind_cols(dta, ptemp) %>% 
